...
async def main():
    ...
    async with (
        ...
        # Cleanup agents before exiting the async with block
        await cleanup_agents(client, agent_incident, agent_devops)
...
class SelectionStrategy(SequentialSelectionStrategy):
    ...
    async def select_agent(self, agents, history):
        ...
        def select_next_agent(self, chat_history: List[ChatMessage]) -> Optional[Agent]:
            # Implement your logic to select the next agent based on the chat history
            return None
...
async def cleanup_agents(client, agent_incident, agent_devops):
    print("Cleaning up agents:")
    try:
        # Delete the agent definitions from the Azure AI service
        await client.agents.delete_agent(agent_incident.definition.id)
        await client.agents.delete_agent(agent_devops.definition.id)
        print("Agents cleaned up successfully.")
    except Exception as e:
        print(f"Error during cleanup: {e}")
        # Don't close the client here - it's managed by the async with block
